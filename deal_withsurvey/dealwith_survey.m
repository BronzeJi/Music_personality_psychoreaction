...
T= readtable('data/da_522.xlsx','Format','auto');% change the input file here
currentFolder = pwd;
     %       ^^^^^^^^^------ your csv filename
%p=T{:,1};
%q=T{:,2};
...
person.MBTI=Musicengineeringanswers(:,4);
person.MBTI(1,:)=[];
%% 1
...
%% 2 Correlation_feeling
%data
P=zeros(1,12); %12 songs
for j=1:12
    for i=1:9 %9 label
        P(i,j)=outResults.("song"+num2str(j))(1,i);
    end
end
clear j
...
%% 3 Correlation_MBTI   (work from Lukas, no idea if it is correct
% %M=zeros(1,12); %12 songs
% for j=1:12
%     M(:,j)=person.song.(genreName{j})(:,9);
% end
% M(:,13)=mean(M,2);
% clear j
% 
% figure(3);
% scatter(1:size(person.MBTI),M(:,13),"filled")
% 
% for j=1:length(person.MBTI)
%     score(j)=emotionMBTIdic(person.MBTI(j));
% end
% % correaltion between MBTI emotion from web and our data
% tempcor=corrcoef(score,M(:,13));
...
% function [score]=emotionMBTIdic(personality)
%     scorelist=[64,63,74,68,89,86,85,86,51,77,58,77,50,74,55,76];
%     personalitylist=["INTJ","INTP","ENTJd","ENTP","INFJ","INFP","ENFJ","ENFP","ISTJ","ISFJ","ESTJ","ESFJ","ISTP","ISFP","ESTP","ESFP"];
% 
%     index = find(personalitylist==personality);
