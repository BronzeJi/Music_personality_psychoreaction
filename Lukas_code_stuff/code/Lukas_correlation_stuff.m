...
T= readtable('Music engineering (answers).xlsx','Format','auto');
currentFolder = pwd;
     %       ^^^^^^^^^------ your csv filename
%p=T{:,1};
%q=T{:,2};
...
Musicengineeringanswers_groups = group_splitter(Musicengineeringanswers);

person.song.Rock = textRemover(Musicengineeringanswers_groups(:,8:16));

person.song.Metal = textRemover(Musicengineeringanswers_groups(:,17:25));

person.song.Techno = textRemover(Musicengineeringanswers_groups(:,26:34));

person.song.EDM = textRemover(Musicengineeringanswers_groups(:,35:43));

person.song.Hardstyle = textRemover(Musicengineeringanswers_groups(:,44:52));

person.song.Jazz = textRemover(Musicengineeringanswers_groups(:,53:61));

person.song.Classical = textRemover(Musicengineeringanswers_groups(:,62:70));

person.song.Pop = textRemover(Musicengineeringanswers_groups(:,71:79));

person.song.RnB = textRemover(Musicengineeringanswers_groups(:,80:88));

person.song.Film = textRemover(Musicengineeringanswers_groups(:,89:97));

person.song.Kpop = textRemover(Musicengineeringanswers_groups(:,98:106));

person.song.Indie = textRemover(Musicengineeringanswers_groups(:,107:115));

person.song.PlayMusic = yesNo_data_remover(Musicengineeringanswers_groups(:,6));
...
for z=1:4  % 12 songs
    %titles = ['Rock','Metal','Techno','EDM','Hardstyle','Jazz','Classical music','Pop','RnB','Cinema','Kpop','Indie'];
    nexttile
    % Figure properties
...
%while contains(Musicengineeringanswers_groups(:,4),'explorers')
for j=1:12
    for i=1:9 %9 label
